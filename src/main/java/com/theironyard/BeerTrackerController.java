...
public class BeerTrackerController {
    ...
    public void init() throws InvalidKeySpecException, NoSuchAlgorithmException {
        User user = users.findOneByName("Landon");
        if (user == null) {
            user = new User();
            user.name = "Landon";
            user.password = PasswordHash.createHash("stuff");
            users.save(user);
        }
    }
    ...
    @RequestMapping("/")
    public String home(Model model, String type, Integer calories, String search, HttpSession session, String showMine) {
